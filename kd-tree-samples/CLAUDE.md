...
**Scene loading** (`common/data/loader/mitsuba3.cu`). The loader consumes a Mitsuba 3 scene XML, resolves `$defaults`, loads referenced `.obj` meshes via tinyobjloader, and produces a `World*` (`common/data/world.cuh`) holding `Model*` (mesh + material), a `std::vector<Light*>` (point / spot / directional, tagged `Light` struct), and a `Camera*`. Materials are simplified to one of `LAMBERTIAN`/`DIELECTRIC`/`CONDUCTOR` (mutually exclusive — combinations from the XML are collapsed).

**Lighting units (physically based).** `Light::power` is RGB radiant intensity in W/sr for point/spot lights and RGB irradiance in W/m² for directional lights — it carries both magnitude *and* hue (no separate color field). Direct lighting is `(ρ/π)·power·attenuation·cos·spot` with `attenuation = 1/d²` (point/spot) or `1` (directional). The emitter converts `power` to total flux `Φ` via a per-light factor (`4π` point, cone solid angle `2π(1−cosOuter)` spot, disk area `πR²` directional) and stores each photon's flux as `Φ/N`, so the path tracer's photon-density estimate is in the same units as the direct term. Don't reintroduce intensity scalars — tune brightness with the scene's `indirect_intensity` (artistic GI gain) and the viewer `exposure`. (`EmittedPhoton::power` is a vestigial `int`, unused by shading.)
